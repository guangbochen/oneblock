...
const (
	...
	huggingFaceHubTokenEnvName = "HUGGING_FACE_HUB_TOKEN"
)
...
func GetRayClusterSpecConfig(mlSvc *mlv1.MLService, modelTmpVersion *mlv1.ModelTemplateVersion, releaseName string) (*rayv1.RayClusterSpec, error) {
	...
	for i, wgCfg := range mlSvc.Spec.MLClusterRef.RayClusterSpec.WorkerGroupSpec {
		workerGroupSpecs[i], err = GetDefaultWorkerGroupSpecConfig(&wgCfg, modelTmpVersion, clusterImg, mlSvc.Spec.HFSecretRef)
		if err != nil {
			return nil, err
		}
	}
...
func GetHeadGroupSpecConfig(mlService *mlv1.MLService, modelTmpVersion *mlv1.ModelTemplateVersion, releaseName, image string) (rayv1.HeadGroupSpec, error) {
	...
	headGroupSpec.RayStartParams = rayStartParams
...
func GetDefaultWorkerGroupSpecConfig(wgCfg *mlv1.WorkerGroupSpec, modelTmpVersion *mlv1.ModelTemplateVersion,
	image string, hfRef *mlv1.HFSecretRef) (rayv1.WorkerGroupSpec, error) {
	...
	if wgCfg.Resources == nil && modelTmpVersion.Spec.WorkerGroupSpec.Resources != nil {
		podTemplate.Spec.Containers[0].Resources = *modelTmpVersion.Spec.WorkerGroupSpec.Resources
	} else if wgCfg.Resources != nil {
...
func configResourceAccelerators(spec *mlv1.WorkerGroupSpec) string {
	...
	if spec.AcceleratorTypes != nil {
		for aType, v := range spec.AcceleratorTypes {
			...
			logrus.Infof("debug accelerator str: %s", accelerators)
		}
